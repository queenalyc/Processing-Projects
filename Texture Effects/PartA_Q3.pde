...
void setup() {
    ...
    materialbw = loadShader("b&w_fragment.glsl");
    materialemboss = loadShader("emboss_fragment.glsl");
    ...
    cp = control.addColorPicker("BlacknWhite_rgba").setPosition(110, 200).setColorValue(color(255, 128, 0, 0));
    cp2 = control.addColorPicker("Emboss_rgba").setPosition(110, 320).setColorValue(color(255, 128, 0, 0));
...
public void controlEvent(ControlEvent e){
    if(e.isFrom(cp)){
        float coeffR = e.getArrayValue(0);
        float coeffG = e.getArrayValue(1);
        float coeffB = e.getArrayValue(2);
        float coeffA = e.getArrayValue(3);
